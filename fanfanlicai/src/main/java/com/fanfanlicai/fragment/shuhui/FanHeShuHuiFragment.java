...
public class FanHeShuHuiFragment extends BaseFragment {

	private PullToRefreshListView refreshListView;
	private ListView listView;
	private CustomProgressDialog progressdialog;
	private ArrayList<ShuHuiBean> list = new ArrayList<ShuHuiBean>();
	private ShuHuiAdapter adapter;
	...
	protected View initView() {
		...
		adapter = new ShuHuiAdapter(mActivity,list);
						...
						if ("0".equals(code)) {
							...
							if (StringUtils.isBlank(datastr)) {
								...
								if (isSuccess) {// 验签成功
									...
									tv_chaxun.setText("提示：平台平均赎回时间为"+recAvgTimeStr);
