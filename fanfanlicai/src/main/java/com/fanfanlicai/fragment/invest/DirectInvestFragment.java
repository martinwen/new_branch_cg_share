...
import com.fanfanlicai.activity.my.DirectInvestDetailActivity;
import com.fanfanlicai.adapter.direct.DirectAdapter;
 ...
 * 已投-饭碗记录（代替老饭碗）
 */
public class DirectInvestFragment extends BaseFragment implements View.OnClickListener {
    ...
    protected View initView() {
        ...
        View view = View.inflate(mActivity, R.layout.fragment_directinvest, null);
        ...
        //要在setTime 之前才有效果哦
        pvTime.setRange(calendar.get(Calendar.YEAR) - 6, calendar.get(Calendar.YEAR) + 14);
        ...
        adapter = new DirectAdapter(mActivity, list);
        listView.setAdapter(adapter);
        listView.setOnItemClickListener(new DirectInvestFragment.ItemListener());
        return view;
    }
    ...
    class ItemListener implements OnItemClickListener {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
            ...
            Intent intent = new Intent(mActivity, DirectInvestDetailActivity.class);
            intent.putExtra("transTime", fwBean.getTransTime());//买入时间
            intent.putExtra("transTimeStr", fwBean.getTransTimeStr());//时分秒 买入时间
            intent.putExtra("yearRate", fwBean.getYearRate());//预期年华率
            intent.putExtra("investAmount", fwBean.getInvestAmount());//投资金额
            intent.putExtra("endTime", fwBean.getEndTime()); //到期时间
            intent.putExtra("investDeadline", fwBean.getInvestDeadline()); // 投资期限
            intent.putExtra("status", fwBean.getStatus());//投资状态 :1：还款中；3：已完成；4：回款中
            intent.putExtra("recInterest", fwBean.getRecInterest());//待收收益
            intent.putExtra("waitRepayAmount", fwBean.getWaitRepayAmount()); // statu=1待收本息；status=3 已结本息
            intent.putExtra("transUuid", fwBean.getTransUuid());//交易流水号
            intent.putExtra("type", fwBean.getType());//新老饭碗分类 1老，2新
