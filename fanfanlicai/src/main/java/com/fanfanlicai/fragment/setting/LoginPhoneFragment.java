...
public class LoginPhoneFragment extends BaseFragment implements View.OnClickListener,TextWatcher {
	...
	protected View initView() {
		phone = CacheUtils.getString(mActivity, CacheUtils.LOGINPHONE, "");
		progressdialog = new CustomProgressDialog(mActivity, "正在加载数据...");

		View view = View.inflate(mActivity, R.layout.fragment_loginphone, null);
		...
		tv_yuyin = (TextView) view.findViewById(R.id.tv_yuyin);// 语音验证码
...
//				if (hasFocus) {
//				} else {
//					LogUtils.i("mima==="+mima);
//					LogUtils.i("Base64.encode(SignUtil.encrypt(mima))==="+ Base64.encode(SignUtil.encrypt(mima)));
//					LogUtils.i("loginPassWord==="+loginPassWord);
//					if (!isMima(mima)) {
	...
	public void onClick(View v) {
		switch (v.getId()){
			...
			case R.id.iv_yanzhengma:// 点击获取验证码，弹出dialog输入提示
				YzmDialog yzmDialog_sms = new YzmDialog(mActivity, R.style.YzmDialog, phone);
				yzmDialog_sms
				...
				break;
			case R.id.tv_yuyin:// 点击获取验证码，弹出dialog输入提示
				...
				yzmDialog_yuyin
						.setOnYzmDialogDismissListener(new YzmDialog.OnYzmDialogDismissListener() {
							...
							public void OnYzmDialogDismiss(String answer) {
								// TODO Auto-generated method stub
								progressdialog.show();
								String token2 = CacheUtils.getString(mActivity, "token", null);
								Map<String, String> map2 = SortRequestData.getmap();
								map2.put("phone", phone);
								map2.put("sendType", 1+"");
								map2.put("codeType", 3+"");
								map2.put("captchaResult", answer);
								map2.put("token",token2);
								String requestData2 = SortRequestData.sortString(map2);
								String signData2 = SignUtil.sign(requestData2);
								map2.put("sign", signData2);
								VolleyUtil.sendJsonRequestByPost(ConstantUtils.SMS_URL, null, map2, new HttpBackBaseListener() {
