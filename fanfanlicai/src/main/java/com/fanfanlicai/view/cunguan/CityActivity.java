...
public class CityActivity extends BaseActivity {
    ...
    private List<City> mDatas = new ArrayList<City>();
    ...
    private void getProvinces() {
        ...
        VolleyUtil.sendJsonRequestByPost(ConstantUtils.LISTCITY_URL, null, map,
                new HttpBackBaseListener() {

                    @Override
                    public void onSuccess(String string) {
                        LogUtils.i("城市列表="+string);
                                ...
                                if (isSuccess) {// 验签成功
                                    ...
                                    for (Object letter : letters) {
                                        ...
                                        for (Object o : jsonArray) {
                                            ...
                                            City city = new City();
                                            city.firstPinYin = obj.getString("cityFirstLetter");
                                            city.id = obj.getString("cityId");
                                            city.cityPinYin = obj.getString("cityLetters");
                                            city.cityName = obj.getString("cityName");
                                    ...
                                    mRecyclerView.setLayoutManager(mLinearLayoutManager = new LinearLayoutManager(CityActivity.this));

                                    mRecyclerView.setAdapter(mAdapter = new BaseRecyclerAdapter<City>(CityActivity.this, mDatas, R.layout.rv_item_city) {
                                        @Override
                                        protected void convert(BaseViewHolder helper, final City item) {

                                            if (helper.getAdapterPosition() == 0) {
                                                tvLetterHeader.setText(item.firstPinYin);
                                            ...
                                            helper.setOnClickListener(R.id.tv_city, new View.OnClickListener() {
                                                ...
                                                public void onClick(View view) {
                                                    ...
                                                    if(1==flags){
                                                        CGBindBankActivity.ADDRESS = province+item.cityName;
                                                        CGBindBankActivity.ID = item.id;
                                                        finish();
                                                    }else if(2==flags){
                                                        CGBankChangeActivity.ADDRESS = province+item.cityName;
                                                        CGBankChangeActivity.ID = item.id;
                                                        finish();
                                                    }
                                                }
                                            });
